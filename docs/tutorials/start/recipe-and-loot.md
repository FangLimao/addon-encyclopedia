# 配方与战利品表
## 配方
配方是游戏中的基础，通过配方可以获得游戏中的各种物品，附加包允许我们通过数据驱动的方式添加自定义配方。

我们来看一下最基础的有序配方的大致格式，在我们的行为包下新建`recipes`文件夹，这个文件夹将会存放我们的自定义配方。

在`recipes`中创建一个JSON文件，写入以下内容：
...
接下来，我们来看一下无序配方的大致格式，其与有序配方相似，但更加简单：
...
在`loot_tables`中创建一个JSON文件，写入以下内容：
